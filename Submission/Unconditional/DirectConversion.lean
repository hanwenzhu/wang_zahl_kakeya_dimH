...
import Submission.Unconditional.AmbientTransport
import Submission.Unconditional.NuminaAnalytic
import Submission.Unconditional.GeometryAdapters
import Submission.Unconditional.CoaxialJohnTransfer
import Submission.Unconditional.FiniteParentRecords
import Submission.Unconditional.FiniteJointGeometry
import Submission.Unconditional.SparseGridSchedule
import Submission.Unconditional.ConversionLogAbsorption
import Submission.Unconditional.ScaleToOrdinaryCWA
import Submission.Unconditional.SingleParentScale
import Submission.Unconditional.CoaxialFixedSupport
import Submission.Unconditional.FiniteParentPreparation
  ...
  have finiteJohnConsumer {I : Type uI} {delta : NNReal} (hdelta : 0 < (delta : ℝ))
      ...
      (parent : Fin
        (canonicalReanchoredParents numinaRepresentativeDilation selection.center
          (toTubeFamily data.selectedParents (input.parentTube coordinate))).card)
      ...
      have hclass (j : I) (hj : j ∈ active) :
        ...
        calc
          ...
          _ ≤ constant * (ratio *
            ...
            mul_le_mul_left (mul_le_mul_right (hmass j hj) constant) _
          _ = _ := by ring
      have hsum :
    ...
    have sourceIndexOriginal (i : Fin sub.family.card) :
      ...
      have hi : originalIndex i ∈ t :=
        Finset.mem_image.mpr ⟨i, Finset.mem_univ i, rfl⟩
      change ((t.equivFin.symm (t.equivFin ⟨originalIndex i, hi⟩) :
        {i // i ∈ t}) : I) = originalIndex i
