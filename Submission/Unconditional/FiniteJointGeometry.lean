...
import Submission.Unconditional.FiniteParentProducers
import Submission.Unconditional.NuminaJointCore
import Submission.Unconditional.SelectionPolylog
import Submission.Unconditional.SpatialLocalization
...
private theorem finite_scale_of_regularized_selection
  ...
  · refine ⟨huniform, ?_⟩
    intro first second
    let first' : Fin selectedParents.card := Fin.cast (by rfl) first
    let second' : Fin selectedParents.card := Fin.cast (by rfl) second
    have fiberCount_eq (parent : Fin coarse.card) :
        cover.fiberCount parent =
          ((Finset.univ.filter fun index =>
            assigned index = Fin.cast (by rfl) parent).card : ENNReal) := by
      simp only [TubeCover.fiberCount]
      apply congrArg (fun fiber : Finset (Fin selection.localized.family.card) =>
        (fiber.card : ENNReal))
      apply Finset.filter_congr
      intro index _
      constructor <;> intro h <;> apply Fin.ext <;>
        simpa only [cover, Fin.cast] using congrArg Fin.val h
    rw [fiberCount_eq first, fiberCount_eq second]
