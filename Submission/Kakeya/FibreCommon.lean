...
theorem mem_fibreIndex_self {σ ρ : ℝ≥0} (hσρ : σ ≤ ρ) {i₀ : ι} (hi₀ : i₀ ∈ s) :
  ...
  classical
  change i₀ ∈ Kakeya.familyIn s (fibreBodies T σ)
    ((T i₀).rescale ρ).toConvexSpaceBody
  exact Kakeya.mem_familyIn s (fibreBodies T σ) i₀
    ((T i₀).rescale ρ).toConvexSpaceBody |>.2
    ⟨hi₀, Tube.rescale_le_rescale_of_radius_le (T i₀) hσρ⟩
...
theorem densityIn_fibre_le_mul_densityIn_node {δ : ℝ≥0} {s : Finset ι}
  ...
  have hcancel : Da ≤ (r : ℝ≥0∞) ^ 2 * (C : ℝ≥0∞) ^ 3 * D := by
    exact (ENNReal.mul_le_mul_iff_left hXY0 hXY_top).mp hstep
