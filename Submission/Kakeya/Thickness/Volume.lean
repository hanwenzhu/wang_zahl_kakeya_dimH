...
public import Submission.Kakeya.Mathlib.MeasureTheory.Simplex
public import Submission.Kakeya.Thickness.Basic
public import Submission.Kakeya.Thickness.InnerSimplex
public import Submission.Kakeya.Thickness.OuterPrism
public import Submission.Kakeya.DimensionN.Volume
...
theorem volume_le_prod_thickness {s : Set E} (hs : Bornology.IsBounded s) :
  ...
  have key : volume (outerPrism rfl hK hKne).carrier
    ...
    intro i _
    rw [outerPrism.thicknesses_eq]
    have hth :
        (⟨thickness ℝ (closure s) (i : ℕ), thickness_nonneg (closure s) (i : ℕ)⟩ : NNReal) =
          ⟨thickness ℝ s (i : ℕ), thickness_nonneg s (i : ℕ)⟩ := by
      apply Subtype.ext
      exact thickness_closure hs (i : ℕ)
    exact
      (congrArg (fun r : NNReal => (r : ENNReal)) hth).trans
