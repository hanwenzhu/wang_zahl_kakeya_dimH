...
public import Submission.Kakeya.Tube.Rescale
...
theorem exists_fineNormalization_core [Nontrivial E] (hdim : Module.finrank ℝ E = 3)
  ...
  have hKcarrier : K'.carrier = L '' P.carrier := by
    dsimp only [K']
    unfold ConvexSpaceBody.affineImage
    rfl
  have hL_image (A : Set E) :
      L '' A = Tτ.rescaleMap (R : ℝ) '' A :=
    Set.image_congr fun y _ =>
      congrArg (fun f : E →ᵃ[ℝ] E => f y) hL
  have hKcarrier_rescaled :
      K'.carrier = Tτ.rescaleMap (R : ℝ) '' P.carrier :=
    hKcarrier.trans (hL_image P)
  have hratio' : volume (ConvexSpaceBody.closedUnitBall : ConvexSpaceBody E).carrier
      ≤ (Cw : ℝ≥0∞) * volume K'.carrier := by
    rw [ConvexSpaceBody.closedUnitBall_carrier]
    exact hKcarrier_rescaled.symm ▸ hratio
  ...
  · refine ⟨?_, ?_, ?_, ?_, ?_⟩
    ...
    · intro i hi
      rw [hshade i hi]
      have hWshade : (𝕎 i).shade = L '' (T i).shade := by
        dsimp only [𝕎]
        unfold ShadedBody.affineImage
        rfl
      exact hWshade.trans (hL_image (T i).shade)
    · intro i hi
      simpa [hWcarrier i hi] using hvol i hi
    · intro x hx
      apply hK'c
      exact hKcarrier_rescaled.symm ▸ hx
