...
public import Submission.Kakeya.DimensionThree.MainLemma1.RevisedSource.SourceAssignedNormalizationW97
public import Submission.Kakeya.DimensionThree.MainLemma1.Rescaling.Hybrid
public import Submission.Kakeya.ConvexBody.DilateWitness
...
theorem axisFoot_family_forward_test_w103 (hdim : Module.finrank ℝ E = 3)
  ...
  have hnonzero : ∀ i ∈ F, volume (W i).carrier ≠ 0 := by
    intro i hi
    change volume ((T i).toConvexSpaceBody.affineImage L.toAffineMap
      L.toContinuousAffineEquiv.continuous).carrier ≠ 0
    rw [ConvexSpaceBody.volume_affineImage (T i).toConvexSpaceBody L
      L.toContinuousAffineEquiv.continuous]
    have hcpos : (0 : ℝ≥0∞) <
        (Tube.le_volume.c (Module.finrank ℝ E) : ℝ≥0∞) :=
      ENNReal.coe_pos.mpr (Tube.le_volume.c_pos (Module.finrank ℝ E))
    have hrpos : (0 : ℝ≥0∞) < (r : ℝ≥0∞) := ENNReal.coe_pos.mpr hr
    have hlower_ne : (Tube.le_volume.c (Module.finrank ℝ E) : ℝ≥0∞) *
        (r : ℝ≥0∞) ^ (Module.finrank ℝ E - 1) ≠ 0 :=
      mul_ne_zero hcpos.ne' (pow_ne_zero _ hrpos.ne')
    have hlower : (0 : ℝ≥0∞) <
        (Tube.le_volume.c (Module.finrank ℝ E) : ℝ≥0∞) *
          (r : ℝ≥0∞) ^ (Module.finrank ℝ E - 1) :=
      lt_of_le_of_ne zero_le hlower_ne.symm
    exact mul_ne_zero (Kakeya.ofReal_abs_det_affineEquiv_ne_zero L)
      (hlower.trans_le (Tube.le_volume (T i))).ne'
  ...
  refine ⟨D, hKD, ?_, ?_⟩
  · have hcarrier :
        (K.affineImage L.toAffineMap L.toContinuousAffineEquiv.continuous).carrier =
          L '' K.carrier := by
      change L.toAffineMap '' K.carrier = L '' K.carrier
      exact Set.image_congr fun _ _ => rfl
    have hscale :
        ENNReal.ofReal (4 * (48 * (R : ℝ))) = ((192 * R : ℝ≥0) : ℝ≥0∞) := by
      calc
        ENNReal.ofReal (4 * (48 * (R : ℝ))) =
            ENNReal.ofReal ((192 * R : ℝ≥0) : ℝ) := by
              congr 1
              rw [← mul_assoc]
              norm_num
        _ = ((192 * R : ℝ≥0) : ℝ≥0∞) := ENNReal.ofReal_coe_nnreal
    rw [hdim, hcarrier, hscale] at hvolD
    norm_num only [Nat.factorial] at hvolD
    simpa only [ENNReal.coe_mul, ENNReal.coe_pow, ENNReal.coe_ofNat] using hvolD
  · intro i hi hiK
    apply hVD i hi
