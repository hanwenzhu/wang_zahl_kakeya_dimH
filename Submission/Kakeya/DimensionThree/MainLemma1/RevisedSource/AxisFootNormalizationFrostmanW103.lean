...
public import Submission.Kakeya.DimensionThree.MainLemma1.RevisedSource.AxisFootNormalizationGeometryW103
...
theorem axisFoot_family_frostman_w103 (hdim : Module.finrank ℝ E = 3)
  ...
  have hP'carrier : P'.carrier = L '' P.carrier := by
    dsimp only [P']
    unfold ConvexSpaceBody.affineImage
    rfl
  have hP'volume : volume P'.carrier =
      ENNReal.ofReal |LinearMap.det (L.linear : E →ₗ[ℝ] E)| * volume P.carrier := by
    rw [hP'carrier]
    exact Kakeya.volume_affineImage L P.carrier
  ...
  have hratio : volume K.carrier / volume P'.carrier <= C := by
    apply (ENNReal.div_le_iff hP'pos P'.isCompact'.measure_ne_top).mpr
    rw [hP'carrier]
    exact hKvol
