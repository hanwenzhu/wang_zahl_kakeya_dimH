...
public import Submission.Kakeya.DimensionThree.MainLemma2.CanonicalCentredCover
public import Submission.Kakeya.DimensionThree.MainLemma1.RevisedSource.RawTreeClosenessW96
...
theorem exists_actual_source_regularized_working_tower_w95
  ...
  have hlocalize {delta : ℝ≥0} (hdelta : 0 < delta) (hdelta1 : delta < 1)
    ...
    let newTube : (k : Nat) -> iota -> Tube (Tube.gridScale delta M k) E :=
      fun k R => match k with
        | 0 => (Tube.gridScale_zero delta M).symm ▸ (Y j0).toTube.rescale 1
        | k + 1 => W (k + 1) R
    have hnewzero : ∀ R, (newTube 0 R).toConvexSpaceBody =
        ((Y j0).toTube.rescale 1).toConvexSpaceBody := by
      intro R
      exact hcastBody (Tube.gridScale_zero delta M).symm ((Y j0).toTube.rescale 1)
    have hnewpos (k : Nat) (hk0 : k ≠ 0) (R : iota) : newTube k R = W k R := by
      cases k with
    ...
    let G : Tube.GridCoverSystem S (fun i => (Y i).toTube) M := {
      ...
      le_tube_assign := by
        ...
        by_cases hk0 : k = 0
        · subst k
          rw [hnewzero (newAssign 0 i)]
          exact (hcover 1 hM (hSF hi)).trans (hfirst_root i hi)
        · rw [hnewpos k hk0 (newAssign k i)]
          simpa only [newAssign, if_neg hk0] using hcover k hk (hSF hi)
      nested := by
      ...
      tube_nested := by
        ...
        · subst k
          rw [hnewzero (newAssign 0 i)]
          rw [hnewpos 1 (by omega) (newAssign 1 i)]
          simpa only [newAssign, Nat.zero_add, if_neg (by omega : 1 ≠ 0)] using hfirst_root i hi
        · rw [hnewpos (k + 1) (by omega) (newAssign (k + 1) i),
            hnewpos k hk0 (newAssign k i)]
          simpa only [newAssign, if_neg hk0, if_neg (show k + 1 ≠ 0 by omega)] using
    ...
    · intro i hi
      change (newTube M i).toConvexSpaceBody = (Y i).toConvexSpaceBody
      rw [hnewpos M hMpos.ne' i]
      exact hbottom (hSF hi)
    · intro k hk a ha b hb hab
      ...
      · have hmem : ∀ R ∈ S.image (newAssign k), R ∈ parent k := by
        ...
        change newTube k a = newTube k b at hab
        rw [hnewpos k hk0 a, hnewpos k hk0 b] at hab
        exact hab
    · intro k hk V
      by_cases hk0 : k = 0
      ...
      · apply (Finset.card_le_card ?_).trans (hoverlap k hk V)
        ...
        change (Y i).toConvexSpaceBody <= (newTube k R).toConvexSpaceBody at hiW
        rw [hnewpos k hk0 R] at hiW
        exact hiW
    · intro k hk R hR
      by_cases hk0 : k = 0
      · subst k
      ...
      · obtain ⟨i, hi, rfl⟩ := Finset.mem_image.mp hR
        ...
        rw [hnewpos k hk0 (newAssign k i)]
        simp only [newAssign, if_neg hk0]
