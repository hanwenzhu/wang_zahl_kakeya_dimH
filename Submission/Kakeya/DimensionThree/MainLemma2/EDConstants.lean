...
public import Submission.Kakeya.DimensionThree.MainLemma2.Conjunct6Refutation
...
theorem sixteen_le_edRadiusConstant : 16 ≤ edRadiusConstant := by
  ...
  have : (2 : ℝ≥0) ≤ edDilateConstant := by
    ...
    have hquot : (0 : ℝ) ≤ 2 * 4 ^ 3 / ((Tube.le_volume.c 3 : ℝ≥0) : ℝ) :=
      div_nonneg (by norm_num) hc.le
    calc
      (2 : ℝ) ≤ 9 := by norm_num
      _ ≤ 9 + 2 * 4 ^ 3 / ((Tube.le_volume.c 3 : ℝ≥0) : ℝ) :=
        le_add_of_nonneg_right hquot
  exact this
