...
public import Submission.Kakeya.DimensionThree.MainLemma2.BallConstruction
...
noncomputable abbrev deleteShade (cfg : VeryNotSticky.{u})
...
noncomputable def BallDataCore.deleteShade (core : BallDataCore cfg)
  ...
  Yg_mass := by
    ...
    have hsumT : ∑ i ∈ cfg.s, volume (deleteShadeTube (cfg.T i) S hS).shade =
        ∑ i ∈ cfg.s, volume (cfg.T i).shade :=
      Finset.sum_congr rfl fun i _ => h1 i
    have hsumY : ∑ i ∈ cfg.s, volume (core.Yg i \ S) =
        ∑ i ∈ cfg.s, volume (core.Yg i) :=
