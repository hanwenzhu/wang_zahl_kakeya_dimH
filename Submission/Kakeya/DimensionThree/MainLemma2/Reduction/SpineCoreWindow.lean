...
public import Submission.Kakeya.DimensionThree.MainLemma2.Reduction.SpineShadingBridge
...
theorem exists_threshold_two_mul_ssfGridLen_le_log :
  ...
  let δ₀ : ℝ≥0 := ⟨Real.exp (-8), (Real.exp_pos _).le⟩
  have hδ₀coe : (δ₀ : ℝ) = Real.exp (-8) := rfl
  refine ⟨δ₀, ?_, ?_, ?_⟩
  · apply NNReal.coe_pos.mp
    rw [hδ₀coe]
    exact Real.exp_pos (-8)
  · apply NNReal.coe_le_coe.mp
    rw [hδ₀coe, NNReal.coe_one]
    rw [Real.exp_le_one_iff]; norm_num
  · intro δ hδ0 hδle
    have hdR : (δ : ℝ) ≤ Real.exp (-8) := by
