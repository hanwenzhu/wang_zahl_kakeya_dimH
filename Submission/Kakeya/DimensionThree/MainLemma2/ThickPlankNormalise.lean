...
public import Submission.Kakeya.DimensionThree.MainLemma2.PlankPresentation
public import Submission.Kakeya.DimensionThree.MainLemma2.PlankRescaling
public import Submission.Kakeya.DimensionThree.MainLemma2.ThickPlankInterface
public import Submission.Kakeya.DimensionThree.MainLemma2.ThickCase
public import Submission.Kakeya.DimensionThree.Plank.TubePlankNormalisation
...
theorem exists_thickBodyPlank {cfg : VeryNotSticky.{u}} (bd : BallData cfg) {B : bd.bι}
  ...
  have hcarrier : (((bd.Wb j).plankRescale (bd.ctr B)
    ...
    rw [hK, hL]
    change
      (ConvexSpaceBody.affineImage
        (plankRescale (bd.ctr B)
          (NNReal.coe_ne_zero.mpr (thickPlank_r₁_pos' cfg).ne')).toAffineMap
        _ (bd.Wb j)).carrier =
          plankRescale (bd.ctr B) (NNReal.coe_ne_zero.mpr (thickPlank_r₁_pos' cfg).ne') ''
            (bd.Wb j).carrier
    unfold ConvexSpaceBody.affineImage
    exact Set.image_congr (by
      intro y _
      simp only [AffineEquiv.coe_toAffineMap, plankRescale_apply])
...
theorem exists_thickPlankNormalisation {cfg : VeryNotSticky.{u}} (bd : BallData cfg)
  ...
  have hKpcar : ∀ p, ((Kp p).carrier : Set E₃) = L₁ '' ((bd.Y p).carrier : Set E₃) := by
    ...
    change
      (ConvexSpaceBody.affineImage
        (plankRescale (bd.ctr B) hr₁.ne').toAffineMap _ (bd.Y p).toConvexSpaceBody).carrier =
          plankRescale (bd.ctr B) hr₁.ne' '' (bd.Y p).carrier
    unfold ConvexSpaceBody.affineImage
    exact Set.image_congr (by
      intro y _
      simp only [AffineEquiv.coe_toAffineMap, plankRescale_apply])
