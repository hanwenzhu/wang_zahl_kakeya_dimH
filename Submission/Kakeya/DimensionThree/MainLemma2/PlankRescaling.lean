...
public import Submission.Kakeya.Thickness.Projection
...
theorem isCRefinement_comap_plankRescale (hr₁ : r₁ ≠ 0) {s' s : Finset ι}
  ...
  · -- shade
    ...
    have hsub := (h.1.2 i hi).2
    change (V' i).shade ⊆
      (plankRescale ctr hr₁).toAffineMap '' (V i).shade at hsub
...
theorem exists_isCRefinement_of_plankRescale (hr₁ : r₁ ≠ 0) {s s' : Finset ι}
  ...
  · intro i hi
    have hVshade : (V'' i).shade = (YP' i).shade := by
      simp only [V'', dif_pos hi]
    have himage :
        ((V'' i).plankRescaleSymm ctr hr₁).shade =
          (plankRescale ctr hr₁).symm.toAffineMap '' (V'' i).shade := by rfl
    rw [himage, hVshade]
