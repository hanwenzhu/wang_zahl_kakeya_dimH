...
public import Submission.Kakeya.Mathlib.Analysis.ProjectivePacking
...
theorem badAgainstSet_of_subset
  ...
  have hc_low_eq : ENNReal.ofReal c_low =
    ...
    calc
      ENNReal.ofReal c_low =
          ENNReal.ofReal ((Tube.le_volume.c n : ℝ) * (δ : ℝ) ^ (n - 1)) := by rfl
      _ = (Tube.le_volume.c n : ℝ≥0∞) *
          ENNReal.ofReal ((δ : ℝ) ^ (n - 1)) :=
        ofReal_c (Tube.le_volume.c n) ((δ : ℝ) ^ (n - 1))
