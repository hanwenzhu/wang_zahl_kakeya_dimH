...
theorem FrostmanEstimate.toTypeZero {β : ℝ} (h : FrostmanEstimate.{v} E β) :
  ...
  have hFrost' : IsFrostmanIn s' (fun x => (T' x).toConvexSpaceBody)
    ...
    intro K' hK'
    simpa only [s', T', densityIn, Finset.sum_map, Finset.filter_map, Function.comp_apply,
      he_down] using hFrost K' hK'
  have hFull' : ShadedBody.fullness s' (fun x => (T' x).toShadedBody) ≥ δ ^ η := by
    simpa only [s', T', ShadedBody.fullness, ShadedBody.fullness', Finset.sum_map, he_down]
      using hFull
