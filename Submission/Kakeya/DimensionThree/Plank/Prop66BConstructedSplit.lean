...
public import Submission.Kakeya.DimensionThree.Plank.Prop66BConstructedOuter
public import Submission.Kakeya.DimensionThree.Plank.GlobalPlankFactorizationEstimate
public import Submission.Kakeya.DimensionThree.Plank.MasterScaleLemma61
public import Submission.Kakeya.DimensionThree.IsometryTransport
public import Submission.Kakeya.DimensionThree.Plank.InnerEDAssembly
...
theorem factoringAndMultPropGlobal_constructed :
  ...
  have hinnerSet'' :
    ...
    apply Finset.ext
    intro i
    rw [Section6PartBData.fineOutput_innerSet,
      Section6PartBData.fineOutput_outerSet, Finset.mem_filter]
    constructor
    · intro hi
      have hiq'' : i ∈ q'' := by simpa only [hq''] using hi
      refine ⟨hiq'', ?_⟩
      have hMembershipEq :
          ((BlockCount.restrictCells D cells' hsub).cellOfFine i ∈ O.outerSet) =
            (((BlockCount.restrictCells
              (BlockCount.restrictCells D cells' hsub) O.outerSet hsubO).withShades
                (fun i => (O.innerBody i).shade)
                (fun i => (O.innerBody i).measurableSet_shade) hYsub).cellOfFine i ∈
              ((BlockCount.restrictCells
                (BlockCount.restrictCells D cells' hsub) O.outerSet hsubO).withShades
                  (fun i => (O.innerBody i).shade)
                  (fun i => (O.innerBody i).measurableSet_shade) hYsub).factor.cells) := by
        rfl
      exact Eq.mp hMembershipEq (BlockCount.mem_restrictFine_iff.mp hi).2
    · intro hi
      simpa only [hq''] using hi.1
  have hbody'' : ∀ i ∈ q'', (
      ((BlockCount.restrictCells (BlockCount.restrictCells D cells' hsub) O.outerSet
        hsubO).withShades (fun i => (O.innerBody i).shade)
  ...
  have hcardTotal :
    ...
    have hfibsub :
      ...
      have hCellEq :
          ((BlockCount.restrictCells (BlockCount.restrictCells D cells' hsub) O.outerSet
            hsubO).withShades (fun i => (O.innerBody i).shade)
            (fun i => (O.innerBody i).measurableSet_shade) hYsub).factor.Cell =
            (BlockCount.restrictCells D cells' hsub).factor.Cell := by
        rfl
      have hcell :
          (BlockCount.restrictCells D cells' hsub).cellOfFine i = cast hCellEq x₀ := by
        exact congrArg (cast hCellEq) hi'.2
      have hiTarget :
          i ∈ (BlockCount.restrictCells D cells' hsub).fineFibre x₀ :=
        (Section6PartBData.mem_fineFibre_iff
          (D := BlockCount.restrictCells D cells' hsub)).mpr
            ⟨hq''q' hi'.1, hcell⟩
      exact hiTarget
    have h2 : (qj.card : ℝ≥0∞)
