...
public import Submission.Kakeya.DimensionThree.Plank.Factorization
...
public import Submission.Kakeya.StickyKakeya.Constants
...
theorem fiber_ofSubset_eq_inter {ι ω : Type*}
  ...
  ext i
  rw [ConvexSpaceBody.FactorFamily.mem_fiber, Finset.mem_inter,
    ConvexSpaceBody.FactorFamily.mem_fiber]
  change (i ∈ s' ∧ F.parent i = j) ↔
    ((i ∈ F.innerSet ∧ F.parent i = j) ∧ i ∈ s')
  constructor
  · rintro ⟨hi, hp⟩
    exact ⟨⟨hs' hi, hp⟩, hi⟩
  · rintro ⟨⟨_, hp⟩, hi⟩
