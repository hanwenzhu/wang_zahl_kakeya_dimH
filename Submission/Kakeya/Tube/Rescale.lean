...
public import Submission.Kakeya.AffineMap
public import Submission.Kakeya.Homothety
public import Submission.Kakeya.Mathlib.Topology.CoveringNumber
...
public import Submission.Kakeya.Tube.EssentiallyDistinctReduction
public import Submission.Kakeya.Tube.IntersectionVolume
...
theorem normalization_apply (T : Tube θ E) (z : E) :
  ...
  simp only [normalization, AffineMap.comp_apply, AffineEquiv.coe_toAffineMap,
    AffineEquiv.constVAdd_apply, LinearMap.coe_toAffineMap, vadd_eq_add]
  rw [neg_add_eq_sub]
  have hlin : (T.normalizationLinear : E → E) =
      fun w => (θ : ℝ)⁻¹ • w +
        (1 - (θ : ℝ)⁻¹) • (inner ℝ T.direction w • T.direction) := by
    funext w
    exact T.dilateAux_apply (θ : ℝ)⁻¹ w
  have hinner :
      T.normalizationLinear (z - T.x) =
        inner ℝ T.direction (z - T.x) • T.direction +
          (θ : ℝ)⁻¹ •
    ...
    calc
      T.normalizationLinear (z - T.x) =
        ...
        congrFun hlin (z - T.x)
      _ = _ := by module
  exact congrArg (fun w : E => T.x + w) hinner
