...
public import Submission.Kakeya.ConvexHull
...
public import Submission.Kakeya.Mathlib.MeasureTheory.Action
public import Submission.Kakeya.Mathlib.Algebra.Div
...
instance (K : ConvexSpaceBody E) : DecidablePred fun i ↦ W i ≤ K := Classical.decPred _
...
@[simp]
theorem mem_familyIn (i : ι) (K : ConvexSpaceBody E) :
    i ∈ familyIn s W K ↔ i ∈ s ∧ W i ≤ K := by
  classical
  simp [familyIn]
...
theorem sum_volume_le_maxDensity_mul_volume' {W : ι → ConvexSpaceBody E}
    {K : ConvexSpaceBody E} (h : ∀ i ∈ s, W i ≤ K) :
