...
import Submission.Kakeya.Tube.Basic -- shake: keep
...
lemma image_segment_toPi {ι : Type*} (x y : EuclideanSpace ℝ ι) :
  ...
  change WithLp.ofLp '' segment ℝ x y = segment ℝ (WithLp.ofLp x) (WithLp.ofLp y)
  exact image_segment ℝ ((WithLp.linearEquiv 2 ℝ (ι → ℝ)).toAffineMap) x y
