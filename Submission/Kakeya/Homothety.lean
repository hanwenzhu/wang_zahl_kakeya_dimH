...
lemma homothety_inv_image_homothety_image (x : E) {r : ℝ} (hr : r ≠ 0) (A : Set E) :
  ...
  rw [← Set.image_comp]
  have hcomp : (AffineMap.homothety x r⁻¹ : E → E) ∘ AffineMap.homothety x r =
      id := by
    funext y
    change AffineMap.homothety x r⁻¹ (AffineMap.homothety x r y) = y
    rw [← AffineMap.homothety_mul_apply, inv_mul_cancel₀ hr, AffineMap.homothety_one]
    rfl
  rw [hcomp, Set.image_id]
