...
public import Submission.Kakeya.Factoring.Step2
...
theorem pointwiseMultiplicity_step3FactorFamily_fiber (F : FactorFamily E ι κ)
  ...
  classical
  simp only [fiberMultiplicity, pointwiseMultiplicity]
  by_cases hcond : x ∈ Ω ∧ x ∈ step3DyadicSet F u t' k j
  · rw [if_pos hcond]
    apply congrArg Finset.card
    ext i
    simp only [Finset.mem_filter, FactorFamily.mem_fiber,
      step3FactorFamily_innerSet, step3FactorFamily_parent,
      step3InnerBody, Set.mem_inter_iff]
    constructor
  ...
  · rw [if_neg hcond]
    ...
    simp only [Finset.mem_filter, FactorFamily.mem_fiber,
      step3FactorFamily_innerSet, step3FactorFamily_parent,
      step3InnerBody, Set.mem_inter_iff] at hi
    apply hcond
    have hparent : F.parent i = j := hi.1.2
...
theorem step3FactorFamily_fiber_multiplicity (F : FactorFamily E ι κ)
  ...
  have hi' : i ∈ {i ∈ {i ∈ u | F.parent i ∈ t'} | F.parent i = j} := by
    have hmem := (FactorFamily.mem_fiber
      (step3FactorFamily F hu t' Ω hΩ k) j i).mp hi
    rw [step3FactorFamily_innerSet, step3FactorFamily_parent] at hmem
    exact Finset.mem_filter.mpr hmem
