...
public import Submission.Kakeya.DimensionThree.InducedShading
public import Submission.Kakeya.Factoring.OuterMultiplicity
public import Submission.Kakeya.Factoring.ProductiveFamilies
public import Submission.Kakeya.Factoring.WeightedPipelineRefinement
...
theorem outerThickFamilyAtScale_innerSet_eq_filter
  ...
  simp only [outerThickFamilyAtScale, productiveThickFamily_innerSet,
    productiveThickFamily_outerSet, mem_productiveFinalInnerSet_iff]
  exact (Finset.mem_filter
    (p := fun i : ι =>
      F.parent i ∈
        (outerThickFamilyAtScale F hδ hdisc D w₁ hw₁).outerSet)
    (s := F.innerSet) (a := i)).symm
...
theorem outerThickFamilyAtScale_fiberMultiplicity_lower
  ...
  have hfiber :
      C.fiber j = (outerThickFamilyAtScale F hδ hdisc D w₁ hw₁).fiber j := by
    dsimp only [C, outerCountingFamilyAtScale, outerThickFamilyAtScale]
    rw [productiveCountingFamily_fiber_eq, productiveThickFamily_fiber_eq]
  have hinner :
      C.innerBody = (outerThickFamilyAtScale F hδ hdisc D w₁ hw₁).innerBody := by
    rfl
  rw [← hfiber, ← hinner]
  exact hlower
