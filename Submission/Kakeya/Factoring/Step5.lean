...
public import Submission.Kakeya.Factoring.Step1
public import Submission.Kakeya.Factoring.Step4
...
noncomputable def step5ShadedFactorFamily (F : FactorFamily E ι κ) {u : Finset ι}
  ...
  shade_subset_parent := fun i hi ↦ by
    ...
    have hiFiber : i ∈ (step5FactorFamily F hu t' Ω hΩ k T' w₁).fiber (F.parent i) := by
      exact (FactorFamily.mem_fiber
        (step5FactorFamily F hu t' Ω hΩ k T' w₁) (F.parent i) i).2 ⟨hi, rfl⟩
    exact fun x hx ↦
